...
display('ALO is optimizing your problem');
tym = cputime;
...
while Current_iter<Max_iter+1
    ...
    %to print the values of Y and X in workspace
    valY=[Elite_antlion_fitness];
    valX=[Elite_antlion_position];%valX=valX.';
    assignin('base','ALO_valX',Elite_antlion_position)
    assignin('base','ALO_valY',Elite_antlion_fitness)
    %to print Y and Iteration no in table
   ...
   %to print the value of the variable in static text
    optimumX = get(handles.text15,'string');
    optimumX=Elite_antlion_position;
    set(handles.text15,'string',optimumX);
   
    Current_iter=Current_iter+1; 
    
    z=[exetym results{2,1} valY valX];
    filename='Book1.xlsx';
    xlswrite(filename,z,'B3:C3:D3:H3');
