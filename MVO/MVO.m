...
function [Best_universe_Inflation_rate,Best_universe,Convergence_curve]=MVO(N,Max_time,lb,ub,dim,fobj,handles)
display('MVO is optimizing your problem');
tym = cputime;
...
while Time<Max_time+1
    ...
    if Time>2
        line([Time-1 Time], [Convergence_curve(Time-1) Convergence_curve(Time)],'Color',[0.4660    0.6740    0.1880])
        xlabel('Iteration');
    ...
    results{3,1}=Time;
    results{3,2}=Best_universe_Inflation_rate;
    set(handles.uitable1,'data',results);
    
    %to print the results of variable x in static text box
    optimumX = get(handles.text15,'string');
    optimumX=Best_universe;
    set(handles.text15,'string',optimumX);
    
    Time=Time+1;
end
    
valX=Best_universe;%valX=valX.';
valY=Best_universe_Inflation_rate;
assignin('base','MVO_valX',Best_universe)
assignin('base','MVO_valY',Best_universe_Inflation_rate)

    z=[exetym results{3,1} valY valX];
    filename='Book1.xlsx';
    xlswrite(filename,z,'B4:C4:D4:H4');
end
